...
# 2. Claude 로 캡션 생성
# ─────────────────────────────────────────────
# 모델이 구조 안내 문구("빈 줄" 등)를 캡션에 그대로 써버린 경우 제거하는 안전장치
_GUIDE_LINE = re.compile(
    r"^\s*(?:\[?\d\]?\s*[).]?\s*)?"
    r"(?:빈\s*줄|한\s*줄\s*띄움|한\s*줄\s*띄우기|줄\s*바꿈|줄바꿈|훅|본문|마무리|해시태그)"
    r"\s*[:：]?\s*$"
)


def clean_caption(text):
    lines = [ln for ln in text.split("\n") if not _GUIDE_LINE.match(ln)]
    out = "\n".join(lines)
    out = re.sub(r"\n{3,}", "\n\n", out)
    return out.strip()
...
아래 [1]~[4] 를 이 순서대로 이어서 쓰고, 덩어리와 덩어리 사이는 엔터를 두 번 쳐서 한 줄 띄웁니다.
※ 매우 중요: 줄을 띄우라는 지시는 실제로 엔터만 치라는 뜻입니다. 안내용 단어를 캡션에 글자로 적으면 안 됩니다.
[1] 첫 줄 (125자 이내): '더보기'를 누르기 전에 보이는 유일한 줄. 스크롤을 멈추게 하는 훅.
   예시 스타일: "서울에서 3시간, 사람 없는 마이산 뷰 호텔", "홍삼스파 하고 자면 다음날 몸이 다름 (진심)"
[2] 본문 짧은 문단 3~5개: 후기·경험담·정보 느낌. 문장은 짧게, 광고 티 금지.
   실용 정보(코스, 소요시간, 꿀팁 등)를 1개 이상 넣어 '저장' 가치를 만들 것.
[3] 댓글·저장·태그를 부르는 마무리 한 줄 (매번 다른 방식):
   · "같이 갈 사람 태그" · "저장해두고 가을에 꺼내보기" · "밸런스 게임: 스파 먼저 vs 등산 먼저"
   · "진안 가본 사람 꿀팁 댓글로" · "이런 여행 좋아하는 사람 손"
[4] 해시태그: 아래 고정 태그를 빠짐없이 모두 넣고, 주제에 맞는 태그 3~5개를 추가 (총 25개 이내)
...
- 캡션 외의 설명·따옴표·머리말 출력
- '빈 줄', '한 줄 띄움', '[1]', '훅', '본문', '마무리' 같은 구조 안내 단어를 캡션에 글자로 적는 것"""
...
위 주제로 인스타그램 캡션 본문만 출력해 주세요."""
    ...
    return clean_caption(text)[:2190]
